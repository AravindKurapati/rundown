...
- **Expressive narration over call-count minimalism.** The first cut narrated a whole episode in one TTS call, which was tidy but read flat: every story at the same pitch. Splitting narration per segment costs the same (billing is per character) and lets the energy the script asked for actually reach the voice, with each call conditioned on its neighbors' text to keep the read continuous. Eleven v3's inline audio tags were the tempting shortcut and were rejected: a 5,000-character cap that cannot fit a ten-minute episode in one call, and no continuity mechanism to chunk around it.
